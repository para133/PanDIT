...
class AnalysisMetrics:
    ...
    def ref_metrics(self, pred, gt):
        return ref_evaluate(pred, gt)


    def batch_metrics(self, pred, gt, mode):
        for i in range(pred.shape[0]):
            pred_img = np.array(ToPILImage(mode=mode)(pred[i]))
            gt_img = np.array(ToPILImage(mode=mode)(gt[i]))
            metrics = self.ref_metrics(pred_img, gt_img)
            for key in metrics:
                self.acc_d[key].append(metrics[key])
                
    def get_metrics(self):
        return {key: mean_std(self.acc_d[key])[0] for key in self.acc_d}
    
    def get_all_metrics(self):
        return {key: mean_std(self.acc_d[key]) for key in self.acc_d}
    
    def get_metrics_str(self):
        msg = ''
        metrics = self.get_all_metrics()
        for key in metrics:
            msg += f"{key}: {metrics[key][0]} ± {metrics[key][1]}\n"
        return msg
